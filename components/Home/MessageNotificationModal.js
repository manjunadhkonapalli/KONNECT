...
function MessageNotificationModal({
    ...
    const formSubmit = (e) => {
        ...
        if(socket.current){
            socket.current.emit('sendMsgFromNotification', {userId: user._id, msgSendToUserId: newMessageReceived.sender, msg:text});

            //After giving reply, the modal will get closed automatically just like whatsapp reply in notification bar
                ...
                <Segment secondary color="teal" attached="bottom">
                    <Form reply 
                        ...
                        <Form.Input 
                            ...
                            action={{color:"blue", icon:"whatsapp plane", disabled:text==="", loading:loading}} />
                    </Form>
                </Segment>
                </div>
